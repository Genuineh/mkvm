...
- Linux server grabbed its own uinput virtual keyboard (`mkvm-keyboard`) along with physical keyboards, creating a feedback loop: every re-emitted key press landed back in MKVM's capture queue and got reprocessed, which killed keyboard forwarding to the remote machine. `discover_mouse_devices` and `discover_keyboard_devices` now skip any device named `mkvm-mouse` or `mkvm-keyboard`.
- Holding a key on Linux produced a single character instead of a run. The kernel sends auto-repeat events with `value=2`, but the capture handler treated anything `value != 1` as a key-up, so each repeat toggled press/release and libinput's repeat timer never settled. Auto-repeat events are now ignored locally (libinput on the `mkvm-keyboard` virtual device does its own repeat from the initial key-down) and forwarded as press when controlling a remote.
