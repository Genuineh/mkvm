...
[![下载](https://img.shields.io/github/v/release/Genuineh/mkvm?label=%E4%B8%8B%E8%BD%BD&style=for-the-badge)](https://github.com/Genuineh/mkvm/releases/latest)
...
[![平台](https://img.shields.io/badge/平台-macOS%20%7C%20Windows%20%7C%20Linux-2786ff?style=for-the-badge)](https://github.com/Genuineh/mkvm/releases/latest)
[![许可证: MIT](https://img.shields.io/badge/license-MIT-green?style=for-the-badge)](./LICENSE)

[English README](./README.md)

![MKVM 演示](docs/screenshots/tour.gif)
...
1. **两台机器都安装。** 从 [最新发布](https://github.com/Genuineh/mkvm/releases/latest) 下载各自系统的安装包。
2. **选择角色。** 在要共享键鼠的那台机器上打开 MKVM，保持 **服务端**（默认）。在另一台机器上打开 MKVM，到设置里切换为 **客户端**。
...
- **macOS（服务端）。** 在 系统设置 → 隐私与安全性 中给 MKVM 同时授予 **辅助功能（Accessibility）** 和 **输入监控（Input Monitoring）**，这是捕获和注入键鼠输入所必需的。签名版本在更新后会保留授权；万一掉了，关掉再打开即可。
- **macOS 首次打开。** 版本是免费自签名（未经过 Apple 公证），所以首次会被 Gatekeeper 拦。右键点应用 → **打开** → **打开** 放行一次即可。
- **Windows。** 常规使用无需特殊权限。只有需要控制提权/管理员窗口时才以管理员身份运行。
- **Linux。** 用 AppImage 的话，先给它加可执行权限（`chmod +x`）。输入捕获/注入需要 `/dev/uinput` 写权限和 `input` 组成员身份，详见下方 **Linux（Wayland）支持** 章节。

## Linux（Wayland）支持

> 这是 MKVM 相对于原 mykvm 的主要新增功能——原版没有 Linux 输入捕获（贴边切换仅支持 macOS/Windows）。

MKVM 在**内核层**使用 [`evdev`](https://docs.rs/evdev) 和 `uinput` 实现输入捕获与注入，与显示服务器无关：可在 **niri 等 Wayland 合成器**下工作（Wayland 屏蔽了显示服务器级别的输入捕获，CGEventTap / X11 XRecord 等价物在 Wayland 下不可用或受限）。

### 工作原理

- **捕获（服务端）：** 打开鼠标的 `/dev/input/event*` 节点并调用 `EVIOCGRAB` 独占抓取——内核不再把事件投给合成器，本地光标冻结，我们读取相对 `REL_X`/`REL_Y` 增量。累加增量得到虚拟光标位置，送入与 macOS/Windows 后端相同的边缘检测逻辑（因此贴边行为跨平台一致）。未控制远端时，已消费的相对事件会通过 uinput 虚拟鼠标重新发出，本地光标照常移动。
- **注入（客户端）：** 收到的远端事件写入 uinput 虚拟键鼠。Windows 虚拟键码与 evdev `KeyCode` 双向映射。

### 配置

```bash
# 1. 加入 input 组（读取 /dev/input/event*）
sudo usermod -aG input $USER

# 2. 通过 udev 规则给 /dev/uinput 写权限（避免每次 sudo）
...
由于捕获/注入发生在内核 evdev/uinput 层，MKVM 可在任何 Linux 会话下工作：**niri、Sway、Hyprland、GNOME（Wayland）、KDE（Wayland）以及 X11**。无需合成器特定的协议支持。

### 已知限制

- **多个鼠标：** 所有检测到的鼠标设备都会被抓取并累加相对增量。实际同时刻只有一个鼠标在动，所以一般无碍。
- **绝对定位注入：** uinput 鼠标是相对设备，注入鼠标移动用上次位置算差值。Wayland 安全模型禁止直接访问绝对光标坐标；但服务端发送的就是相对移动语义，实际表现正常。
- **边缘本地光标隐藏：** 与 macOS 不同，Wayland 下抓取时无法可靠隐藏本地光标。抓取期间光标停在边缘不动（冻结，不移动）——视觉上可接受，但不如 macOS 无缝。
- **协议兼容性：** MKVM 保留原 `mykvm.*.v1` 协议标记，与原 mykvm 线兼容。MKVM 服务端可控制 mykvm 客户端，反之亦然。

## 已知限制

- **仅限可信局域网。** 暂无用户配对/PIN，且局域网发现是明文、未认证。请勿把端口暴露到公网或不可信网络。
- 输入和剪贴板走 **加密的 QUIC/TLS** 连接，并绑定对端广播的证书；但 MKVM 仍是原型，未针对恶意网络做加固。
- 剪贴板同步 **文本和图片**，不同步文件。
...
MKVM 是一个实验性的早期版本，适合在本地可信网络中测试和迭代，但还没有面向不可信网络做生产级加固。当前版本和安装包见 [Releases 页面](https://github.com/Genuineh/mkvm/releases)。
...
MKVM 使用两条通道：局域网发现走普通 UDP 端口；输入和剪贴板走另一个 UDP 端口上的加密 QUIC 连接。
...
QUIC 连接使用 TLS 1.3 加密：每个端在启动时生成一张自签名证书并在发现阶段广播，连接方会 pin 住这张证书，因此输入和剪贴板流量是加密的、且只绑定到广播该证书的对端。发现通道本身仍是明文且未认证，因此请把 MKVM 保持在可信局域网内使用。
